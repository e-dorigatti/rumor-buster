...
import os, math, re, sys, fnmatch, string, csv
import tweepy
import json
import nltk
...
f = 'sentiment_lexicon.txt' # empirically derived valence ratings for words, emoticons, slang, swear words, acronyms/initialisms
try:
    WORD_VALENCE_DICT = make_lex_dict(f)
except:
    f = os.path.join(os.path.dirname(__file__),'sentiment_lexicon.txt')
    WORD_VALENCE_DICT = make_lex_dict(f)
...
if __name__ == '__main__':
    ...
    i = 0
    with open('all-tweets.jsonl') as f:
                    for row in f:
                        tweet = json.loads(row)
                        if i <= 50:
                            if detect(tweet['_source']['text'])=='en' and detect(tweet['_source']['text'])!="":
                                #print (tweet['text'])
                                try:
                                    vs = sentiment((tweet['_source']['text']).encode('utf8'))
                                except:
                                    pass
                                analysis = TextBlob(tweet['_source']['text'])
                                print (analysis.tags)
                                saSubjectivity = analysis.sentiment.subjectivity
                                ...
                                #print(vs.compound)
                                english_text_file = codecs.open("NRC-emotion-lexicon-wordlevel-alphabetized-v0.92.txt", "r", encoding="utf-8")
                                for line in english_text_file.readlines():
                                    if not line.isspace():
                                        spl = line.strip().split("\t")
                                        #print (spl)
                                        word, emotion, value = spl[0], spl[1],spl[2]
                                        #sampletweet = 'zeal'
                                        if word in tweet and int(value) >= 1:
