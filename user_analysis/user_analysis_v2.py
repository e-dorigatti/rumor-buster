...
plotly_username = 'mileshu'
access_token = 'pk75t32gva'
...
class UserAnalytics():
    ...
    def user_followers_following_statistics(self):
        ...
        print 'self.user.create_at: ',  self.user.created_at
    ...
    def get_months(self,user_created_date):
        ...
        print type(self.user_created_date)
    ...
    def user_feature_without_timeline(self):
        """ Function to analysis user statistics without timeline """
        notimeline_statistics_dict = {}
        notimeline_statistics_dict['user_following_count'] = self.user.friends_count
        notimeline_statistics_dict['user_followers_count'] = self.user.followers_count
        notimeline_statistics_dict['created_date'] = self.user.created_at
        print 'self.user.create_at: ',  self.user.created_at
        datetime_data = self.get_months(self.user.created_at)
        notimeline_statistics_dict['number_of_monts'] = datetime_data['months']
        notimeline_statistics_dict['statuses_count'] = self.user.get('statuses_count')
        notimeline_statistics_dict['favourites_count'] = self.user.get('favourites_count')
        notimeline_statistics_dict['listed_count'] = self.user.get('listed_count')

        ''''   
        notimeline_statistics_dict['time_zone'] = self.('user.time_zone')
        notimeline_statistics_dict['user_verified'] = self.user.verified
        notimeline_statistics_dict['geo_enabled'] = self.user.geo_enabled
        notimeline_statistics_dict['default_profile'] = self.user.default_profile
        notimeline_statistics_dict['is_translator'] = self.user.is_translator
        notimeline_statistics_dict['protected'] = self.user.protected 
        '''

        return notimeline_statistics_dict
...
def main():
    ...
    notimeline_statistics_ = useranalytics.user_feature_without_timeline()
    print statistics_
    print notimeline_statistics_
